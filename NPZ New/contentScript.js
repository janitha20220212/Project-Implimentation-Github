...
let postLinks = Array.from(document.querySelectorAll('a[data-testid="post-title"]')).map(a => a.href);
...
window.addEventListener('scroll', function() {
  postLinks = Array.from(document.querySelectorAll('a[data-testid="post-title"]')).map(a => a.href);
  console.log(postLinks);
});
 ...
 function flagHomePosts(){
  ...
  for (let i = 0; i< posts.length; i++){
      if(posts[i].slot.includes("full-post-link")){
          ...
          posts[i].insertAdjacentHTML("afterend", "<button class='flag' style='transform: translate(290px, 32px)'>Flag Post<button/>")
...
function getData(et){
  ...
  try {
      ...
      text = document.getElementsByTagName('h1')[0].parentElement.children[3].textContent.trim()
      heading = document.getElementsByTagName('h1')[0].textContent.trim()
      link = document.getElementsByTagName('h1')[0].parentElement.getAttribute('content-href')
  ...
  let post = document.getElementsByTagName("span");
      ...
      for (let i = 0; i< post.length; i++){
          if(post[i].innerText.includes("Sort")){
              ...
              executed = true;
              console.log("Added a button")
              post[i].insertAdjacentHTML("afterend", "<button class='flag' style='transform: translate(290px, -61px)'>Flag Post<button/>")
